...
def rescale_img(img_in, scale):
    ...
    img_in = img_in.resize(new_size_in, resample=Image.Resampling.BICUBIC)
    return img_in
...
class DatasetFromFolder(data.Dataset):
    def __init__(self, label_dir, data_dir, patch_size, data_augmentation, transform=None):
        ...
        self.data_filenames = [os.path.join(data_dir, f) for f in os.listdir(data_dir) if is_image_file(f)]
    ...
    def __getitem__(self, index):
        ...
        input = load_img(data_path)
        target = load_img(label_path)

        input = input.resize((256, 256), Image.Resampling.BICUBIC)
        target = target.resize((256, 256), Image.Resampling.BICUBIC)
